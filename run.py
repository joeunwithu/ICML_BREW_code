...
def parse_args():
    ...
    parser.add_argument("--min-length", type=int, default=530)
...
def run_algorithm(algorithm_name, transformers_config, samples):
    ...
    with tqdm(total=len(samples), desc=f"Running {algorithm_name}", ncols=100) as pbar:
        for sample in samples:
            ...
            detect_result_watermarked = watermark.detect_watermark(prompt, watermarked_text)
            detect_result_unwatermarked = watermark.detect_watermark(prompt, unwatermarked_text)
            detect_result_natural = watermark.detect_watermark(prompt, natural_text)
            save_text_result(
